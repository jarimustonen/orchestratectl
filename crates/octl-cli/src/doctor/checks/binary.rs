...
//! invoked from an orchestratectl source checkout, compare it with that
//! checkout's `HEAD`.
//!
//! A mismatch is advisory: released binaries and branch work commonly differ
//! legitimately. Outside an orchestratectl checkout there is no meaningful
...
fn result_for(build_commit: &str, reference: RepoHead) -> CheckResult {
    match reference {
        ...
        RepoHead::Head(head) => CheckResult::warn(
            ...
            "confirm that this is the intended binary for the current orchestratectl checkout",
        )
        .with_details(json!({
            "binary_commit": build_commit,
            "repository_head": head,
        ...
        RepoHead::Unavailable => CheckResult::ok(
            "binary.commit",
            format!(
                "running binary commit {build_commit}; orchestratectl repository HEAD unavailable"
            ),
        )
        .with_details(json!({
            "binary_commit": build_commit,
...
fn repository_head() -> RepoHead {
    ...
    if !is_orchestratectl_checkout(&top) {
        return RepoHead::NotApplicable;
    }
...
fn is_orchestratectl_checkout(root: &Path) -> bool {
    let workspace = root.join("Cargo.toml");
    let cli_manifest = root.join("crates/octl-cli/Cargo.toml");
    if !workspace.is_file() || !cli_manifest.is_file() {
        return false;
    }
    let Ok(body) = std::fs::read_to_string(cli_manifest) else {
        return false;
    };
...
mod tests {
    ...
    fn this_checkout_is_recognised() {
        ...
        assert!(is_orchestratectl_checkout(root), "{}", root.display());
    }
}
