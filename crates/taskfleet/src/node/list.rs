...
use taskfleet_core::{read_manifest_opt, read_node_opt, RunLock};
...
pub fn run(args: Args<'_>) -> Result<(), CliError> {
    ...
    let collected = RunLock::with_shared_lock(&paths.lock(), || {
        ...
        let entries = match std::fs::read_dir(&nodes_dir) {
            ...
            Err(e) => return Err(taskfleet_core::Error::io(&nodes_dir, e)),
        };
        if let Some(read) = entries {
            ...
            for ent in read {
                ...
                // truly empty run.
                let ent = ent.map_err(|e| taskfleet_core::Error::io(&nodes_dir, e))?;
                let p = ent.path();
