...
//! `taskfleet_core::append_and_apply_idempotent`: it scans the existing event log,
...
use taskfleet_core::{ensure_root, AppendOutcome, Event, NodeId, RunLock, RunPaths};
...
struct CreatedPayload<'a> {
    ...
    /// them via [`taskfleet_core::plan_projections`] — the single source of truth,
...
/// [`taskfleet_core::plan_projections`] rather than re-enumerated here. Used by
...
fn projected_paths(
    ...
    let abs = taskfleet_core::plan_projections(paths, &ev).map_err(from_core)?;
    Ok(abs
        .iter()
        .map(|p| {
...
pub fn run(args: Args<'_>) -> Result<(), CliError> {
    ...
    let (seq, idempotent_replay) = if let Some(key) = idempotency_key.as_deref() {
        let outcome = RunLock::with_lock(&paths, |lock| {
            taskfleet_core::append_and_apply_idempotent(
                &paths,
                lock,
                kind,
