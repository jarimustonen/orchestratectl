...
use taskfleet_core::{read_all_events, read_manifest, read_node, RunId, RunLock, RunPaths};
...
fn validate_quiescent_root(root: &Path) -> Result<ValidatedRoot, CliError> {
    ...
    for (id, dir) in &run_dirs {
        ...
        for event in &events {
            taskfleet_core::validate_event(&paths, event).map_err(from_core)?;
        }
        ...
        for entry in nodes.into_iter().flatten() {
            ...
            let stem = entry
                ...
                .to_string();
            let node_id = taskfleet_core::NodeId::parse_str(&stem)
...
fn write_receipt(path: &Path, r: &Receipt) -> Result<(), CliError> {
    ...
    let tmp = path.with_extension(format!("json.tmp.{}", taskfleet_core::new_op_id()));
    let mut o = OpenOptions::new();
    o.create_new(true).write(true);
    taskfleet_core::nofollow(&mut o);
    let mut f = o
        .open(&tmp)
