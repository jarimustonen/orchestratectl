...
fn interrupted_create_never_publishes_a_zero_node_run() {
    ...
    let retry = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
...
fn concurrent_retry_refuses_while_creator_lease_is_live() {
    ...
    let mut creator = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
        .env("OCTL_CREATE_SH", &create_sh)
        .args([
    ...
    let orphan_retry = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
        .env("OCTL_TEST_SKIP_MATERIALIZE", "1")
        .env("OCTL_IDEMPOTENCY_PUBLISH_WAIT_MS", "0")
...
fn retry_repairs_published_child_missing_parent_edge() {
    ...
    let interrupted = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
        .env("OCTL_TEST_SKIP_MATERIALIZE", "1")
    ...
    let again = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
        .env("OCTL_TEST_SKIP_MATERIALIZE", "1")
        .args(args)
...
fn run_create_fails_loud_when_supervisor_never_confirms() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_taskfleet"))
        .env("TASKFLEET_HOME", home.path())
        .env("HOME", home.path())
        .env("OCTL_CREATE_SH", &create_sh)
