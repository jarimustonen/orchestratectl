...
cargo insta test --review -p taskfleet --release --test envelope_snapshots
...
```sh
INSTA_UPDATE=always cargo test -p taskfleet --release --test envelope_snapshots
```
...
```sh
cargo test -p taskfleet --release --test envelope_snapshots
```
