...
fn telemetry_has_no_event_projection_outcome_wait_merge_retry_or_cleanup_effect() {
    ...
    assert_eq!(node_after.status, taskfleet_core::Status::Running);
...
fn telemetry_symlinks_are_rejected_without_touching_targets() {
    ...
    assert!(matches!(
        ...
        Err(TelemetryError::Core(taskfleet_core::Error::SymlinkSubdir {
            name: "telemetry",
            ..
        }))
    ));
...
fn bulk_read_localizes_advisory_errors_and_crash_tail() {
    ...
    .unwrap();
    let rows = taskfleet_core::read_all_telemetry_with_clock(&paths, &FixedClock(time(1))).unwrap();
    ...
    .unwrap();
    assert!(matches!(
        taskfleet_core::read_all_telemetry_with_clock(&paths, &FixedClock(time(1))),
        Err(TelemetryError::RunStateNotCurrent)
    ));
}
