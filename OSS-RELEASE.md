...
targets:
  ...
  - {ecosystem: rust, package: octl-core, registry: crates.io, adapter: cargo-dist}
versioning: semver
...
- **targets: two crates.io packages via cargo-dist** — both `octl-core` and `orchestratectl`
  are `publish = true`. The release adapter is **cargo-dist** (not the solo-Rust default
  `cargo-publish`): `.github/workflows/release.yml` is dist-autogenerated, `dist-workspace.toml`
  pins `cargo-dist-version = "0.28.2"`, and `Cargo.toml` carries a `[profile.dist]`. Evidence
