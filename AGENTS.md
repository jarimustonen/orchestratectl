...
- **Release cadence (default posture, since 2026-08-04).** This project **ships real releases, often** — cut one whenever something production-ready lands, don't batch changes into a big release. Two channels, both driven from the approved `OSS-RELEASE.md` contract: **crates.io** via `cargo publish` (`octl-core` first, then `orchestratectl` — the CLI depends on `octl-core = "=<version>"`), and **prebuilt binaries + the per-tool Homebrew tap `jarimustonen/orchestratectl`** via cargo-dist, triggered by pushing a `vX.Y.Z` tag (CI builds the mac target on the self-hosted `hauis` runner). End state: `brew install jarimustonen/orchestratectl/orchestratectl`. The `/oss-release` skill orchestrates the whole thing; mechanics live in `OSS-RELEASE.md` + `dist-workspace.toml`. Before a release, finalize `CHANGELOG.md` (`[Unreleased]` → dated version).
- **In-tool "deploy" (a stint's local reflection).** Distinct from a release: to reflect a CLI-surface / SKILL change in the *running* binary, do a **local rebuild** — `cargo install --path crates/octl-cli --force && orchestratectl skill install --force && orchestratectl doctor` (expect 0 fail / 0 warn).
- **Release / deploy autonomy — fully autonomous, no ask (Jari, 2026-08-05).** local rebuild is always fine. **Pushing `main` is always allowed for this project (no ask)** — this deliberately overrides the global "never push without being asked" default for orchestratectl. **Cutting and publishing a release is ALSO fully autonomous now: the agent may `cargo publish` to crates.io AND push a `vX.Y.Z` release tag without asking, and may decide independently when a release is warranted** (per the release-often cadence above). The former `/oss-release` **approval boundary is removed** for this project — there is no remaining human-confirmation gate on the two release actions. The agent still executes them *correctly and deliberately* (right version, changelog finalized, `octl-core` published before `orchestratectl`, publish dry-run green, tree clean) — autonomy means no permission prompt, not less care. crates.io publishes remain permanent (yank-only), so verify before publishing; that is a correctness duty, not an approval gate.
...
6. **`run merge` is a recorded, OID-recoverable transaction — never a raw git-then-append**
   ...
   `run merge` spans two durability domains — git refs and the event log — and is not atomic across them. A crash after the git merge but before the terminal `explicit-merge` `node.report` would strand the work *merged in source* with *no merge event* (a false `failed`). So `run merge` records a `merge.started` transaction (`op_id`, `expected_source_oid`, `worker_oid`, source/worker branch, driver pid → `Node.pending_merge`) **before** mutating git; merge.sh guards the source-ref fast-forward with a **compare-and-swap** against `expected_source_oid` (exits `76` → `merge_source_moved` if the target moved); and recovery (next `run merge` retry, or the supervisor tick via `merge_recovery::recover_run`) resolves that **one** transaction by exact OID — **complete** it (append the missing `explicit-merge` report) when the source ref moved AND the worker's content is git-verified integrated (rebase-robust, via `run::landed`), or **reject** it (`merge.aborted`, work preserved) when the source ref is unchanged or moved without the worker's content. It is deliberately NOT a general branch-content heuristic (that is the deleted git-reconcile probe) — it only ever finishes/rejects a transaction it already recorded, and only when the driver process is confirmed dead so a live merge is never raced. A failed/conflicted merge aborts its own transaction so no `pending_merge` dangles. Don't reintroduce a broad "branch is an ancestor of source ⇒ done" inference, and don't append `explicit-merge` without the CAS-guarded git mutation having demonstrably landed.
