...
- **Verifying a publish landed — `ossctl release verify <run-id>`.** Registry reconciliation is ossctl's job; do not hand-roll a crates.io API probe (the API rejects a request with no `User-Agent` by returning an empty result, which reads as "not published" and has already caused one false alarm).
