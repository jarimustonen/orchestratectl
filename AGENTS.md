...
- **Release / deploy autonomy:** local rebuild is always fine (no ask). The **irreversible release steps are the human's call** (Jari's global CLAUDE.md): a `cargo publish` to crates.io and pushing `main` / a release tag. Drive the release through `/oss-release` up to its approval boundary (bump chosen, changelog finalized, plan sealed / tag prepared) and hand the publish + push to the human — never publish or push without being asked.
- **Staying in sync (no ask).** You may always run a `pull → rebase → pull` sequence when needed to reconcile local `main` with `origin/main` — `git pull --rebase` (or `git fetch && git rebase origin/main`, then `git pull` again to confirm nothing landed meanwhile). Parallel worktree sessions push under you, so bringing local up to date before a round, before finalizing a handoff, or before preparing a release is expected and safe (it rebases your local commits onto the remote). This is a read/sync operation — it needs no asking. It is NOT the same as pushing: publishing your commits to the remote is still the human's call.
