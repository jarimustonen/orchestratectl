...
5. **Supervisor is the canonical worktree + tmux teardown actor**
   ...
   **Teardown is gated on the terminal outcome — unmerged work preserves the branch + worktree** (the typed outcome table `supervise::outcome::TerminalOutcome::teardown` + the source-relative check, issues `blocked-report-deletes-branch` / `typed-supervisor-outcomes`). Two layers:
   ...
   - **Dirty-worktree guard (uncommitted work):** the source-relative check only protects *committed* work. On the same non-explicit-merge paths, `cleanup_node` also runs `worktree_is_clean` (`git status --porcelain`) **before** `worktree remove --force`; a dirty tree — or one whose status can't be read (fail-closed) — preserves BOTH worktree and branch (`cleanup.branch_preserved`, reason `uncommitted changes in worktree`), so an agent's mid-edit uncommitted scratch is never silently discarded on a cancel/plain-success teardown (issue `non-merge-teardown-dirty-worktree`). Only a confirmed explicit `run merge` (`Teardown::Full`) skips both guards and force-removes — `--force`'s throwaway-scratch removal now applies only where a merge confirmed the work landed.
