...
- **Release / deploy autonomy:** local rebuild is always fine (no ask). **Pushing `main` is always allowed for this project (no ask)** — this deliberately overrides the global "never push without being asked" default (Jari, 2026-08-05) for orchestratectl. The only steps still gated are the two genuinely irreversible / public release actions, which go through the `/oss-release` **approval boundary**: a `cargo publish` to crates.io (permanent) and pushing a `vX.Y.Z` release tag (fires the public binary + Homebrew CI release). For those, confirm the exact version + that the publish is intended, then complete them; routine `main` pushes need no such confirmation.
- **Staying in sync (no ask).** You may always run the `pull → rebase → push` sequence to keep local `main` reconciled with `origin/main` and published — `git pull --rebase` (or `git fetch && git rebase origin/main`), then `git push`. Parallel worktree sessions push under you, so bringing local up to date and pushing before/after a round, before a handoff, or before preparing a release is expected and safe (it rebases your local commits onto the remote first). Both the sync and the push need no asking — only the irreversible release actions above keep their one `/oss-release` confirmation.
