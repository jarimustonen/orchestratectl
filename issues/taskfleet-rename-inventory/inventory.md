...
- **Private self-exec/readiness P until separately versioned:** `OCTL_READINESS_FD` (`run/supervisor_readiness.rs`, `run/supervisor_spawn.rs`, `supervise/mod.rs`) and the profile-launch continuation pair `OCTL_INTERNAL_WORKER_STATE_ROOT` / `OCTL_INTERNAL_WORKER_AWAIT_PUBLICATION` (`run/spawn.rs`, `run_worker.rs`). They cross exec boundaries inside one release; the worker pair is removed before the selected candidate starts.
- **Internal/test seams F:** `OCTL_CREATE_SH`, `OCTL_MERGE_SH`, `OCTL_SUPERVISE_BIN`, `OCTL_READY_WAIT_MS`, `OCTL_PID_FILE_WAIT_MS`, `OCTL_DEATH_GRACE_SECS`, `OCTL_AWAITING_INPUT_GRACE_SECS`, `OCTL_CHILD_SPAWN_DEADLINE_SECS`, `OCTL_NO_WORKER_GRACE_SECS`, `OCTL_STILLBORN_LIST_GRACE_SECS`, `OCTL_WATCHDOG_GRACE_SECS`, `OCTL_AGENT_RESPAWN_MAX_FAILURES`, `OCTL_AGENT_RETRY_MAX_ATTEMPTS`, `OCTL_AGENT_RETRY_BACKOFF_SECS`, `OCTL_TMUX_RETRY_BACKOFF_MS`, `OCTL_IDEMPOTENCY_PUBLISH_WAIT_MS`, and `OCTL_TEST_*` (including dispatcher tests' `OCTL_TEST_SELF_EXE` and `OCTL_TEST_WORKER_PUBLICATION_WAIT_MS`). They are not branded public configuration; retain them to avoid needless test/control churn and because ADR 0002 defaults all `OCTL_*` to stable.
