...
- When a spawned worker dies (`agent-died`) but `orchestratectl run wait`'s `recoverable_work` shows committed, cleanly-merging commits on a **preserved** branch, the conductor must NOT hand-merge that unreviewed work from the orchestrator session.
...
- Primary home: `crates/octl-cli/skills/stint/SKILL.template.md` (Phase 3). Possibly also a shorter note in `worktree-spinoff`'s death-handling section.
