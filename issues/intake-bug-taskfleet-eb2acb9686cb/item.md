...
`taskfleet run wait <run-id> --output json` returns an envelope whose `.data`
terminal fields are all `null` even when the run has settled:

    $ taskfleet run wait 01m02nchnq8awsdmwbz2evz7c4 --output json | jq -c \
...
`taskfleet run show <run-id> --output json` immediately afterward returned them fully
populated:
...
surprising. Observed with taskfleet 0.1.8 on macOS (darwin 25.5.0), 2026-08-16.
