...
- Optional: a `make`/CI helper that reaps stray `taskfleet supervise` processes after the test suite.

## Repro

Run `cargo test --all` (or just `-p taskfleet-cli --test spawn_all_kinds`) a few times, then `pgrep -fl 'taskfleet supervise'` — orphans remain after the run completes.
...
**Product hardening (`crates/taskfleet-cli/src/supervise/mod.rs`).** Orphan defense checked at the TOP
...
`write_json_atomic_no_create` to taskfleet-core and switching `state::save` to the non-creating variant —
...
signals a recycled/foreign PID), replacing the racy `run cancel` cleanup. The `TASKFLEET_TEST_SKIP_MATERIALIZE`
