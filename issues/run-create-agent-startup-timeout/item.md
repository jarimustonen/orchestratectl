...
_Source: crates/taskfleet-cli/src/run/spawn.rs_

## Description

## Summary

`taskfleet run create` does not expose or forward create.sh's
...
loaded", but taskfleet's `spawn.rs::run_create_sh` only forwards `--type`,
...
Consider a higher default than 30s for taskfleet specifically (e.g. 90s), since
taskfleet spawns are frequently part of high-fan-out batches that self-load the
host. CLI-surface change: update insta snapshots + skill.rs catalog pin if
the flag appears in help output; check `taskfleet doctor`.

## Workaround in use

Point `TASKFLEET_CREATE_SH` at a thin wrapper that execs the real create.sh
