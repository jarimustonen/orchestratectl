...
```
$ taskfleet node show n-0001 --run-id 01m05tmnyc2nkmjdqwrqhfwrv6 --output json
{}
```

The documented form is `taskfleet node show <run-id> <node-id>`. So the call above is
...
(`~/.taskfleet/runs/<id>/events.jsonl`) established that the report existed and its
...
taskfleet 0.2.0 (macOS arm64). Reproduced against a completed spinoff run.

## Comments

### 2026-08-17T08:14:16Z · @orchestrator

Closed cannot-reproduce 2026-08-17: verified against the RUNNING 0.2.2 binary, not the issue text. `taskfleet node show n-0001 --run-id <id> --output json` now returns {"schema_version":1,"error":{"code":"unknown_subcommand_or_flag",...}} with exit 1, naming the usage line. The report was filed against 0.2.0. Reopen if an empty {} with exit 0 is observed on 0.2.2 or later.
