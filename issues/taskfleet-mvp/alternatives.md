# taskfleet MVP — Alternatives considered and rejected
...
**Strategy:** No long-lived process at all. TUI (if any) is the only long-lived reader. Agents are detached in tmux and write to disk; their reports sit in `events.jsonl` until a human runs `taskfleet spinoff list` or similar.
...
**Revisit if:**
- The decision-report acting becomes "user runs `taskfleet process-reports` explicitly when they feel like it" — then the supervisor's role evaporates. Considered and rejected because that's worse UX.

### A2.2 — Single global daemon (`taskfleetd`)

**Proposers:** gemini-3.1-pro-preview, gpt-5.5, claude-opus-4-7.

**Strategy:** One long-lived `taskfleetd` process owns all runs, all locks, all child processes. CLI is a thin client over a Unix socket. Auto-started on first CLI invocation, supervised by launchd/systemd.
...
**Revisit if:** Never likely for taskfleet. The recursive supervisor design IS the per-run daemon, just bounded in scope.

### A2.3 — File-watcher daemon (optional, push-only) + stateless writes

**Proposer:** deepseek-v4-pro.

**Strategy:** Writes remain short-lived CLI shell-outs. A separate optional `taskfleet watch` daemon subscribes to filesystem events via `kqueue`/`inotify` and pushes diffs to the TUI over a socket. Pure UX optimization.
...
### A3.1 — Declarative manifest (`taskfleet apply -f run.yaml`)
...
**Why rejected:**
- AGENTS-AI-FIRST-CLI §6 explicitly restricts `apply` to cases where convergent reconciliation is real semantics, not aesthetic. taskfleet creates one-shot runs with no convergence semantics — re-`apply`-ing a "create this run" manifest is just a duplicate.
...
### A3.2 — Verb-first flat surface (`taskfleet list-runs`, `taskfleet spawn-spinoff`)

**Strategy:** Like older Unix tools; no noun layer, just verbs.

**Why rejected:**
- taskfleet has many distinct resources (runs, nodes, events, discussions, spinoffs). A flat verb surface multiplies verbs (`list-runs`, `list-nodes`, `list-events`, `list-discussions`, `list-spinoffs`) and breaks predictability.
...
- Cutting it removes a crate (`taskfleet-tui`), two dependencies, and an entire interaction model from MVP.
...
- Risk of drift between taskfleet's Rust implementation and the existing skill family during the coexistence period.
...
- Library-mode embedding couples taskfleet to upstream workmux releases; every upstream bump risks breakage.
...
**Why this choice:** AGENTS-AI-FIRST-CLI §15 requires the skill subcommand. Having the mechanics in MVP means the path is open for the eventual replacement of the `/worktree-*` skill family with taskfleet-shipped skills.

**Alternative:** Defer entirely until ready to write the full skill library. Rejected because adding the subcommand later means taskfleet's binary has a structural gap (no skill installer); building the mechanics now is small and forward-compatible.
