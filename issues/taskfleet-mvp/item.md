...
Define and ship the MVP of taskfleet: CLI command surface, file-based state schema under `~/.taskfleet/runs/<run-id>/`, and the minimum TUI navigation needed to begin replacing the `/worktree-*`, `/orchestrate`, and `/fan-out` skill family. Parent epic for `design.md`, `breakdown.md`, and the child issues that implement each MVP slice.
...
- Backwards-compatibility with the prose skills' internal state. They run in parallel until the binary is stable; both write into `~/.taskfleet/runs/` once the shim lands.
...
1. **Schema + scaffolding** — `Cargo.toml`, two-crate workspace (`taskfleet-core`, `taskfleet-cli`), on-disk schema frozen in `design.md`.
...
- [ ] @state-schema-crate — taskfleet-core schema types + flock primitive
...
- [ ] @supervisor-process — `taskfleet supervise` long-lived
...
Built in parallel with the existing skills — both write into `~/.taskfleet/runs/` once a thin shim from the skills lands. Until then, the binary's state is the only source of truth and skills are untouched.
