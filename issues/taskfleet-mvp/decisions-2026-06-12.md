# taskfleet MVP — kerätyt päätös­pyynnöt 2026-06-12
...
**Miksi tämä on tärkeää**: Vaikuttaa siihen miten taskfleet:n supervisor reagoi spawn-epäonnistumiseen ja millaista UX:ää tulee.
...
## B3. taskfleet-skill — Install/Upgrade-ohjeistus

**Status**: Muokkaa skill-jakeluun liittyvää konseptia.

**Tilanne**: Pyyntösi: skill taskfleet:lle joka toimii samalla logiikalla kuin issuectl:n skill — sisältää install/upgrade-ohjeet niin että agentti homebase-repossa osaa katsoa `taskfleet --help`:n ja sen avulla tehdä homebasen oikeanlaiseksi. issuectl:llä on osio `## Install or upgrade issuectl`.

**Vaihtoehdot**:
1. **Tee taskfleet-skilliin sama muoto** kuin issuectl:llä: brew, cargo, shell-installer, version-tarkistus, skill-refresh-ohjeet.
2. **Tee skill yksinkertaisempana** koska taskfleet on agent-only — agentti ei välttämättä tarvitse "install or upgrade" -kappaletta jos jakelu hoidetaan vain yhdellä kanavalla (esim. cargo install).
...
- `taskfleet-cli/src/error.rs::SCHEMA_VERSION` (CLI-envelope, eli `--json`-vastauksen muoto)
- `taskfleet-core/src/lib.rs::STATE_SCHEMA_VERSION` (on-disk state-tiedostot)

Molemmat = 1. Reviewerit (anthropic + openai) huomauttivat että CLI-envelope-kontrakti on jaettava (skill-installer, daemon, host-UI kaikki kuluttavat sitä), joten se kuuluu `taskfleet-core`:en, ei CLI-binäärin sisään.

**Vaihtoehdot**:
1. **Siirrä `taskfleet-core`:en heti**. Halvempi nyt.
2. **Jätä CLI-binääriin** kunnes tarvitaan toista kuluttajaa.
3. **Luo `taskfleet-proto` -kraatti** jaetuille envelope-tyypeille (raskaampi, mutta puhtaampi siirtyminen daemoniin).
...
**Tilanne**: Pedantic-clippy päälle/pois? `#![warn(missing_docs)]` taskfleet-coreen?

**Suositukseni**: Anna `ci-and-lints`-issuen agentin tehdä järkevä default-valinta (pedantic warn, missing_docs warn vain taskfleet-core:lle).
