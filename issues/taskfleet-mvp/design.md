...
- Cross-file consistency across a crash is healed by an **`applied_seq` watermark** in `manifest.json` rather than left as a gap. The watermark is the highest event `seq` whose projection fold is durably committed; the writer advances it (fsync) only *after* every projection an event touches is fsynced, so the event log may run ahead of the projections but never the reverse. On the next lock acquisition (any writer or supervisor restart), unapplied tail events (`seq > applied_seq`) are replayed through the reducer before any new append, and an idempotency-key replay catches the projection up before returning the prior event. The reducer's existence/terminal guards make re-folding an already-applied event a no-op, so replay never double-counts. The event log remains canonical (a full `rebuild-projections` from scratch is still a possible future tool); the watermark makes the everyday append+apply pair effectively atomic. A `manifest.json` written before the field existed reads as `applied_seq = 0` (additive serde default) and self-migrates on its next write via a one-time idempotent full replay — no schema bump. **Reducer integrity on replay:** every id the reducer reads off `events.jsonl` (`discussion_id`, `proposal_id`, `child_run_id`, `child_node_id`) is re-validated through its strict `parse_str` before being joined onto a path, so a corrupt log, a restored backup, or a future writer that bypasses the CLI validators can never traverse out of the run dir (the typed-id path constructors only accept already-validated ids — a `"../escape"` id never becomes a `DiscussionId`). Such a poison event is a *valid `Event` envelope* (only its `data` is bad), so the quarantine pass — which excises only lines that fail the strict envelope parse — cannot heal it; replay therefore **skips** it with a `warn` (does not advance the watermark for it, does not panic) rather than hard-erroring, so one tainted line can't permanently brick every future append. The append gate stays fail-closed: the reducer rejects such an event before it is ever written, so a sanctioned log never contains one. See issue `reducer-path-traversal-defense`.
...
- Advisory `flock` is cooperative: a non-conforming script that writes the file directly without locking can still corrupt state. Mitigation: skill-shim documentation explicitly bans direct file writes; `taskfleet event create` is the only sanctioned path.
...
Root: `~/.taskfleet/` (overridable via `$TASKFLEET_HOME`).
...
└── logs/
    └── taskfleet.log.jsonl  # binary's own structured log
```
...
- `index.json` is a cache — if missing or stale, `taskfleet run list` walks `runs/` and rebuilds it.
...
```json
{
  ...
  "source_repo": "/Users/jari/Sources/taskfleet",
  "source_branch": "main",
  "worktree_root": "/Users/jari/Sources/taskfleet.worktrees",
  "node_count": 1,
  "open_discussions": 0,
  "pending_spinoffs": 0,
  "parent_run_id": null,
...
- `lifecycle`: enum `autonomous | interactive`.
  ...
  - `interactive` runs (code) wait for human interaction; agent may self-terminate but typically requires explicit `taskfleet run cancel` or a `/worktree-merge`-style closure. Watchdog tolerates the agent process living indefinitely.
...
- `parent_run_id` / `parent_node_id`: when a child run is spawned by a parent agent via `taskfleet run create` from inside another run, these fields record the spawning context. This is what makes the recursive supervisor model coherent — every run knows its parent.
...
```json
{
  ...
  "worktree_path": "/Users/jari/Sources/taskfleet.worktrees/01jx-n0001",
...
- `children`: array of `{run_id, node_id}` for child runs spawned by *this node's* agent. Populated when this node calls `taskfleet run create --parent-run-id ... --parent-node-id <this>`. Critical for parent-supervisor restart recovery (§7.6).
...
**Decision:** Resource-oriented `taskfleet <noun> <verb>` subcommand tree, modeled after the Linear CLI and git's verb-centric ergonomics. Strict AI-first conventions per [`AGENTS-AI-FIRST-CLI.md`](../../AGENTS-AI-FIRST-CLI.md) bind everywhere.
...
**Trade-offs accepted:**
- Verbose for humans: `taskfleet run create --kind spinoff --title "..."` is more typing than a manifest-apply model (`taskfleet apply run.yaml`). Manifest mode is deferred to post-MVP; AGENTS-AI-FIRST-CLI §6 explicitly limits `apply` to cases where convergent reconciliation is real semantics, not aesthetic.
...
**Signal handling** (§12): long-running streaming commands (`event tail --follow`, `taskfleet supervise`) trap `SIGINT` and `SIGTERM`. SIGINT exits 130; SIGTERM exits 143. A final `{"event":"cancelled","schema_version":1}` event is emitted to stdout before exit when feasible.

### 2.1 Run

- `taskfleet run create --kind <kind> --title <s> [--source-repo <path>] [--source-branch <s>] [--task <s>] [--parent-run-id <id>] [--parent-node-id <id>] [--idempotency-key <opaque>] [--dry-run]` → prints `{schema_version, run_id, dir, supervisor_pid}` on success. `--kind` accepts any of the 8 active kinds; unknown rejected with structured error. Behavior diverges based on whether this is a **top-level run** or a **child spawn**:
  - **Top-level (no `--parent-*` flags):** the CLI initializes the run directory, writes `run.created` + `node.created` to the new run's events, spawns the supervisor (`taskfleet supervise <run-id>`), and exits. Supervisor takes over from there.
...
- `taskfleet run list [--status <s>] [--kind <s>]`
- `taskfleet run show <run-id>`
- `taskfleet run cancel <run-id> [--dry-run]` → marks status `cancelled`, emits `run.status: cancelled` event, **synthesizes a terminal `node.report` event** for each non-terminal node in the run with `{success: false, reason: "cancelled by user", cancelled: true}` so parent supervisors waiting on a terminal report do not hang. Signals the supervisor process to exit gracefully. Domain verb (§2.0).
- `taskfleet run reattach <run-id> [--dry-run]` → restarts the supervisor for an existing run whose `supervisor.pid` is stale or whose process has exited. Reads the run's node JSONs (especially `children`) to rebuild the in-memory tail-follow loops for any in-flight children. Replays unprocessed `node.report` events from children whose `last_processed_report_seq_by_child` lags. Domain verb (§2.0).

### 2.2 Node

- `taskfleet node list <run-id>`
- `taskfleet node show <run-id> <node-id>`
- `taskfleet node report <run-id> <node-id> --from-file report.json [--idempotency-key <opaque>] [--dry-run]` — agent self-submission of structured terminal report (domain verb, §2.0). Payload schema in §7.3. `--idempotency-key` defends against agent retries (two `node report` calls with the same key produce one `node.report` event). When omitted, no deduplication and a second call would create a second report event.

### 2.3 Event

- `taskfleet event tail <run-id> [--from-seq <n>] [--follow] [--format=text|json|jsonl] [--output <FILE>]` — streams run events.
...
- `taskfleet event create <run-id> --kind <k> --node-id <n> --from-file <data.json> [--idempotency-key <opaque>] [--dry-run]` — sanctioned write path for skill-shim and external tools that don't want to manage `flock` themselves. The binary acquires the run's `flock`, assigns `seq`, appends the event, **runs the reducer to update the affected projection files atomically (e.g., a `discussion.opened` event updates `discussions/<id>.json` and `manifest.json.open_discussions`)**, fsyncs, releases the lock. Without the reducer step, sanctioned-write callers would produce drift between the canonical event log and the projection files that read CLIs depend on. Validates `kind` against the known event-kind set; unknown rejected.

### 2.4 Discussion

- `taskfleet discussion list <run-id> [--status open|resolved]`
- `taskfleet discussion show <run-id> <discussion-id>`
- `taskfleet discussion resolve <run-id> <discussion-id> --choice <s> [--note <s>] [--dry-run]` — domain verb (§2.0).

### 2.5 Spin-off

- `taskfleet spinoff list <run-id>`
- `taskfleet spinoff approve <run-id> <proposal-id> [--issue-slug <s>] [--dry-run]` — domain verb (§2.0).
- `taskfleet spinoff reject <run-id> <proposal-id> [--reason <s>] [--dry-run]` — domain verb (§2.0).

### 2.6 Version

- `taskfleet version [--json]` → returns `{schema_version, version, commit, state_schema_version, supported_state_schemas: [1]}`. The `state_schema_version` is the writable schema; `supported_state_schemas` lists schemas this binary can read.

### 2.7 Skill (companion-skill installer)

Per AGENTS-AI-FIRST-CLI §15, taskfleet ships its own AI-operating-manual skills alongside the binary. Skill files live in-repo under `crates/taskfleet-cli/skills/` and ship with the binary.

- `taskfleet skill list [--json]` → lists shipped skills with one-line descriptions.
- `taskfleet skill show <name> [--json]` → prints the skill content without installing.
- `taskfleet skill install [<name>] [--target <dir>]` → copies the skill(s) into `~/.claude/skills/` (or `--target`). Installs all when no name given.

MVP scope ships the **subcommand and mechanics**, with a minimal seed skill set (`taskfleet-run-overview`, `taskfleet-spawn-spinoff`) that proves the path. The full skill library (replacing `/worktree-*` family) lands as a follow-up once the CLI is stable and the surface won't churn under the skill texts.

### 2.8 No TUI subcommand in MVP

The `taskfleet tui` subcommand from the first-pass draft is removed from MVP scope. See §3.

## 3. TUI — deferred (Fork 4)

**Decision:** No TUI in MVP. The human navigates runs via CLI and `event tail --follow`.

**Why:** TUI was the largest scope item in the first-pass draft and the source of all read-frontier pressure (the ~600 file reads/sec analysis). Cutting it removes ~one child issue, a whole crate (`taskfleet-tui`), two dependencies (`ratatui`, `crossterm`), and the entire polling/refresh design surface. Post-MVP a TUI can be added as a separate binary or subcommand reading the same canonical state — the schema is the contract, the TUI is the renderer.

**Trade-offs accepted:**
- Human navigation is uglier in MVP — `taskfleet run list --json | jq` rather than j/k cursor movement.
...
├── Cargo.toml
└── crates/
    ├── taskfleet-core/        # schema types, file I/O, locking, event create, supervisor protocol
    └── taskfleet-cli/         # clap-based CLI, --json plumbing, supervisor entry-point
```

Top-level `Cargo.toml` is a workspace; `cargo install --path crates/taskfleet-cli` produces the `taskfleet` binary. The `taskfleet supervise <run-id>` subcommand re-enters the same binary as a supervisor process — no separate binary needed.
...
The "self-monitoring scripts" pattern that the existing `/orchestrate` skill writes today is essentially this supervisor — taskfleet just standardizes it as a typed Rust process.
...
- After machine reboot, stale `supervisor.pid` files exist for runs whose supervisors are gone. Reattach is explicit via `taskfleet run reattach <run-id>` (§2.1, §7.6). The first read of such a run via `run show` reports the stale state honestly; it does not silently revive.
- Skill-shim path remains independent: bash code shelling `taskfleet event create` does not require a live supervisor — the lock is filesystem-resident. Note: report **consumption** does require the parent supervisor; if the parent is dead, reports queue on disk until reattach.
...
The CLI invocation `taskfleet run create --parent-run-id <P> --parent-node-id <PN> ...` (called by an agent from inside a worktree to spawn a child) is the trigger. The protocol is:
...
- **Interactive lifecycle** (`code`): agent waits for human input in tmux. May self-terminate (with a higher confirmation threshold inside its prompt) or be closed by the human via `taskfleet run cancel` or by a `/worktree-merge`-style script that ends with calling `taskfleet run cancel` or `taskfleet node report` on the agent's behalf. Watchdog is lenient: long-lived agent process is normal; only an explicit terminal event closes the run.
...
**`node.heartbeat` (optional)**: for very-long-running interactive agents, the agent skill may periodically emit `taskfleet event create --kind node.heartbeat`. The supervisor records `last_heartbeat_at`; absence of both heartbeat and PID for > N minutes flags the node as `stalled` in projections (not failed; the human can decide). MVP does not mandate heartbeat for any kind; it's a future opt-in.
...
`taskfleet run reattach <run-id>` restarts the supervisor:
...
3. Spawns a new `taskfleet supervise <run-id>` process.
...
`taskfleet run cancel <run-id>` is a domain verb (§2.0). Beyond marking `status: cancelled`, it **synthesizes a `node.report` event** for every non-terminal node in the run:
...
**Decision:** In MVP, the supervisor's `spawn` step delegates to `~/.claude/skills/worktree/scripts/create.sh` with the appropriate `--type` argument. Post-MVP, the ~150 lines of `create.sh` logic can be ported into Rust as `taskfleet-core/src/spawn.rs` without changing the on-disk contract.

**Why:** The skill family's tmux naming convention (emoji + branch name), workmux integration, agent command resolution, and `--layout` handling already live in `create.sh`. Shelling out keeps a single source of truth during the coexistence period — `taskfleet run create --kind code` and `/worktree-code` produce identical windows and worktrees. No duplication, no drift.

**Trade-offs accepted:**
- Runtime dependency on `~/.claude/skills/worktree/scripts/create.sh` being present and readable by the user running `taskfleet`. Documented in install instructions; supervisor errors with a clear diagnostic if the script is missing.
...
- **Cross-project dependency**: `create.sh` itself must be patched to emit structured stdout (§8.1). The patch is small (~10 lines of bash) and benefits both the skill family and taskfleet; tracked as a `validation.md` dependency that must land before `breakdown.md` issue 10 (`all-kinds-spawn`) can be implemented.
