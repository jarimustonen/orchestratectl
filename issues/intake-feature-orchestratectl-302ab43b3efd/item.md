...
After a spinoff settles, `orchestratectl run show <run-id> --output json` returns `.data.report` (and `.data.nodes[0].report`) as the string `"none"`, even though the worker DID submit a terminal report via `run merge`. The actual report (summary, discussion_items, spinoff_proposals, wrap_up_recommendations) is stored on disk at `~/.orchestratectl/runs/<run-id>/nodes/n-0001.json` under the `.last_report` key. `orchestratectl node show "<run-id>:n-0001" --output json` also returned empty for `.data.report.*` in this session.
...
orchestratectl 0.1.8, macOS. Hit repeatedly this session (4 spinoffs) while gathering terminal-report follow-ups for a stint handoff — the `landed` boolean IS surfaced correctly by `run show`/`run wait` (that part works great); only the structured report is not. Minor friction, not a blocker (the data is on disk), hence `feature` not `bug`.
