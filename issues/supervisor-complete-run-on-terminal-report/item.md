...
Symptom: an autonomous-kind worker submits a terminal `node report` (success or failure), the node correctly reaches `status: done`, but the **run** never transitions to a terminal status and the per-run supervisor process never exits. `taskfleet run show` keeps reporting `status: pending` indefinitely and the supervisor keeps polling — the exact dangling symptom of `spinoff-must-submit-node-report`, which the SKILL fix alone does NOT resolve.
...
- `taskfleet-core/src/reducer.rs::reduce_node_report` updates only the NODE projection. It does not roll the terminal node up to the run manifest status, and emits no `run.status` event.
- The supervisor's `all_work_done` (`taskfleet-cli/src/supervise/mod.rs`) keys off the **manifest** `status` being `Done|Failed|Cancelled`. Nothing sets it for the success path.
- `run.status` events are produced ONLY by `run cancel` (`taskfleet-core/src/cancel.rs`). There is no producer on the agent-success / node.report path.
