...
Symptom: `/orchestrate` skill §5 (decision logging) and §6 (pakkopysäytys) both tell the orchestrator to call `orchestratectl event create --kind <X>` where `<X>` is `orchestrator.decision` or `discuss.critical`. The binary's event-kind enum REJECTS both:
...
Reported 2026-06-28 by an agent running /orchestrate on a real campaign in the deutschpad repo (commit 849d658, skills 0.0.1). The agent worked around it by writing decisions to a plain markdown file (`~/.orchestratectl/runs/<id>/decisions.md`), but the SKILL explicitly states "A choice that does not appear in report.yaml is a contract violation" — and there is no path to make that happen with the current binary.
...
2. Add dedicated CLI verbs: `orchestratectl decision add ...` and `orchestratectl pakkopysaytys open ...`. More explicit; harder for SKILLs to drift from.
...
Acceptance:
- `orchestratectl event create <run-id> --kind orchestrator.decision --from-file <json>` succeeds.
