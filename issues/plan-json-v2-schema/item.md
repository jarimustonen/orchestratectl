...
epic). Behavior-preserving: a new `octl_core::plan` module + checked-in schema
...
- **Serde types** (`Plan`, `Feature`, `Baseline`, `Acceptance` check/assertion
  ...
  in `crates/octl-core/src/plan.rs`.
...
- **Checked-in JSON Schema** (`crates/octl-core/schemas/plan.v2.schema.json`,
