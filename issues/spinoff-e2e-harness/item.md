...
1. Run a real `orchestratectl run create --kind spinoff` against a real (throwaway) git repo and a real (throwaway) `~/.orchestratectl/` root.
...
- New file: `crates/octl-cli/tests/e2e_spinoff.rs` (or a separate `crates/octl-e2e/`).
- Test fixture:
  - `tempfile::TempDir` for throwaway git repo (`git init`, an initial commit, a `main` branch).
  - `tempfile::TempDir` for the throwaway `~/.orchestratectl/` root, exposed via `OCTL_ROOT_DIR` env (or whatever the binary respects).
  - A throwaway tmux session name (`octl-e2e-<pid>`) so the test doesn't touch the developer's main session.
...
Alternative: rather than stubbing the agent, the harness could call a fixed helper script (`scripts/e2e-spinoff-worker.sh`) that does the merge + node.report sequence, bypassing the agent entirely. That tests the orchestratectl + supervisor + cleanup chain without the agent-side LLM dependency.
...
- Headless flag (`orchestratectl-headless-spawn` issue's verification).
...
- `orchestratectl-headless-spawn` — uses this harness for verification
