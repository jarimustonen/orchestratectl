...
- **Milloin tämä näkyy käyttäjälle**: kun `taskfleet run show` (tai TUI) lukee `manifest.json`:in ja `nodes/*.json`:in samalla kun toinen prosessi suorittaa reduceria (esim. `discussion.opened` joka päivittää sekä `discussions/<id>.json`:in että `manifest.open_discussions`-laskurin).
