...
Fix direction (needs design, touches the hot supervise/cleanup path — do NOT bolt in): pin the observed HEAD in a supervisor-owned rescue ref (e.g. `refs/orchestratectl/rescue/<run>/<node>`) before non-force removal, with a retention/GC policy and operator visibility; and/or a worktree lease/lock honored by all orchestratectl writers. Analogous to the deferred durable-operation-lease work in invariant 6 (design §2.7). Scope: closes the committed-HEAD-movement window that non-force removal cannot.
