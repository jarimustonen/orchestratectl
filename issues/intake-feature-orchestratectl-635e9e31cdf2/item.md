...
source_ref: agent:aggountant-wrapup/reporter:jari/id:aggountant-2026-08-28-octl-run-show-source-repo
---
...
`orchestratectl run create` accepts `--source-repo`, but `orchestratectl run show --output json` does not expose the persisted source repository.
