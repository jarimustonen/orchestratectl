...
- `octl_core::nofollow(&mut OpenOptions)` applies `O_NOFOLLOW` via `OpenOptionsExt::custom_flags(libc::O_NOFOLLOW)` on Unix (macOS + Linux + BSD), no-op elsewhere. Wired into every run-state open that touches the leaf: `events.jsonl` append + torn-tail rewrite, the `.lock` open, the atomic temp-file create, and the projection reads (`read_json`/`read_json_opt`). Projection *writes* already go via temp-file + rename and never open the leaf.
- `supervisor.pid` (CLI-owned, `crates/octl-cli/src/supervise/pid_file.rs`): `symlink_metadata` reject before write (`pid_file_symlink_rejected`), `create_new` + `O_NOFOLLOW` temp write, and `O_NOFOLLOW` reads (a symlinked pid file reads as `None`, never followed). Closes `@supervisor-pid-symlink-containment`.
