...
- Codex / multi-agent workflows where the user runs taskfleet in the background.
...
- **taskfleet**: does NOT currently expose either. `spawn::SpawnRequest` has `layout`, `no_hooks`, `keep_tmux_on_error` — no `parent_session` or `headless`.

Fix direction:

1. Add to `taskfleet_cli::run::RunAction::Create` (clap):
...
6. Verification:
   - `taskfleet run create --kind spinoff --headless --title test --task ...` — window appears in `tmux list-windows -t headless`, not `default`. `tmux attach -t headless` shows it.
...
**Live verification** (against the updated create.sh via `TASKFLEET_CREATE_SH`,
