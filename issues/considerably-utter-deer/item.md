...
During the 2026-08-21 taskfleet stint, the repository-authorized local deploy sequence failed twice in the same way.

After `cargo install --path crates/taskfleet-cli --force --locked` completed successfully and replaced `~/.cargo/bin/taskfleet`, the immediately following `taskfleet version --output json` reported an old build commit and caused the mandatory commit-equality gate to fail. Running the exact same version command moments later, without reinstalling or modifying any file, reported the newly installed expected commit.
...
```text
Installing taskfleet v0.4.1 (...)
Finished release profile
Replacing /Users/jari/.cargo/bin/taskfleet
Replaced package ...
expected=fa3a81a239f62a79d688df66043ec4ab715890d2
actual=8777b2e3c5b891abf396c6486c9e81e17ffcfe85
```

Immediately afterward, both `taskfleet version` and `./target/release/taskfleet version` reported `fa3a81a239f62a79d688df66043ec4ab715890d2`. Re-running the provenance check succeeded, followed by skill install and doctor (1131 ok / 0 warn / 0 fail).
...
Recurrence on 2026-08-23 after installing HEAD f268f884035391888b6ec9984bd84c2fa3ac7954. The authorized chained deploy completed `cargo install --path crates/taskfleet-cli --force --locked`, then exited 1 at the immediate expected-vs-actual commit equality test before `ls`, skill install, or doctor. The chain did not print the mismatching actual value. An explicit diagnostic probe immediately afterward resolved `/Users/jari/.cargo/bin/taskfleet` and reported the expected f268f884 commit without any reinstall or file mutation. The failed attempt remains a failed deploy; this note records the recurrence before any fresh attempt.
