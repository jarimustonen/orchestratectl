...
Observed (orchestratectl 0.2.2, issuectl repo, 2026-08-17, run 01m07e2m4nxsmm6wqqtcdsybh5):
a spinoff worker completed Phase A of its task, printed "I stopped before Phases B-D, so I did
not close or merge the run" in its pane, and went idle WITHOUT calling `orchestratectl run merge`.
From the orchestrator's side this state was invisible:

- `orchestratectl run show <id> --output json` reported `status: pending`, `lifecycle: autonomous`
