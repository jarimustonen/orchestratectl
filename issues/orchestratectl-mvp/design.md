...
- `tmux_identity`: fully-qualified `{socket, session, window_id}` captured at spawn (see §8.1) so the watchdog matches the exact window instead of a bare name. `null` for nodes registered before `create.sh` emitted the qualified fields — those fall back to bare-name matching on `tmux_window`. `socket` may be `null` (default-socket spawn); `window_id` is the stable `@NNNN` form that survives renames.
...
- **Qualified tmux identity** (`tmux_socket`, `tmux_session`, `tmux_window_id`) — added for the watchdog's liveness probe. The bare `tmux_window` *name* is not unique across sessions, and a `tmux list-windows -a` on the default socket is blind to windows on other sockets/servers — so a name-only match yields both false-positives ("agent alive" when it's a different session's same-named window) and false-negatives ("agent dead" on a non-default socket). The qualified triple pins the exact window: `tmux_window_id` is the stable `@NNNN` form (survives renames, unique per server), `tmux_session` scopes the lookup, and `tmux_socket` (`#{socket_path}`, or `null` if create.sh could not read it) selects the server. The supervisor folds these into `nodes/<n>.json` as `tmux_identity` (`{socket, session, window_id}`), and `watchdog::tmux_window_present_qualified` matches on `tmux [-S <socket>] list-windows -t <session> -F '#{window_id}'`. **Back-compat:** all three are optional. A create.sh that predates them emits only `tmux_window`; such nodes get `tmux_identity: null` and the watchdog falls back to the legacy bare-name match (with a one-time warning). `tmux_window` is retained for human display.
