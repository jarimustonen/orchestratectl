...
After `taskfleet run cancel <id>` on a `--headless`-spawned run, the worktree and branch are removed but the tmux window stays in the detached `headless` session. The deployed `create.sh` does not emit the **qualified tmux identity** (`session:window_id` form) the supervisor needs to find the window when it lives outside the supervisor's own session.
