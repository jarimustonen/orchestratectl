...
**Scope (this cut only):**
- Delete the code-pipeline subsystem: `crates/taskfleet-cli/src/pipeline/*` and `crates/taskfleet-cli/src/floor/*`, and any CLI surface / commands that only exist to drive them.
...
**Constraints:**
- Do NOT touch `crates/taskfleet-cli/src/skill.rs` or the pi-provenance path — a parallel worktree owns it this round.
...
- Keep `taskfleet-core/{events,lock,reducer,schema}.rs` and `supervise/*` behavioural changes to the minimum needed to unwire the deleted modules (ideally none).

**Acceptance:** green gate (`cargo fmt --all`, `cargo clippy --workspace --all-targets` no new warnings, `cargo test --workspace`) + the insta snapshot loop for any changed CLI surface; `taskfleet --harness pi` and `--harness claude` still launch. Close the now-obsoleted pipeline/harness fix issues (`pipeline-hardening`, `pipeline-run-create-wiring`, `pipeline-breaker-inflight-and-opus-metering`, `pipeline-drop-primitive-underspecified`, `pipeline-tiered-triage`, `dreadfully-dirty-pain`, `practically-exclusive-celery`) as `obsolete` (superseded by this cut). Production code → run `/llm-review` (+ `/assess-findings`) before merging.
