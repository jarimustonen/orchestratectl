...
# pi.dev background jobs extension for taskfleet waits
...
## Shape
Create a separate open source pi.dev extension repo, likely `pi-background-jobs`, rather than vendoring pi-specific event-loop code into taskfleet.

Boundary:
- `taskfleet`: remains the run-state owner and exposes `run wait`, `landed`, and JSON contracts.
...
- First adapter: `/orx-wait <run-id>` runs `taskfleet run wait <run-id> --output json --timeout 6h` and injects a completion message with status, landed flag, and summary.

## Rationale
In pi.dev, a normal bash/tool `taskfleet run wait` blocks the agent turn. A shell-backgrounded `run wait &` can finish, but pi receives no internal event. Extension-owned waiters are the correct integration point because pi can track the child process and wake the agent with `sendUserMessage(..., { deliverAs: "followUp" })`.
