...
~1.5s backoff, overridable via `OCTL_TMUX_RETRY_BACKOFF_MS`). create.sh already rolls
...
is owned by the out-of-repo `/worktree` skill family, so the octl-side bounded retry is
