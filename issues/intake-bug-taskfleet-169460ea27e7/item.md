...
`taskfleet run list` accumulates runs stuck at `status: pending` that never materialized —
...
still unsettled before wrapping up":
    ...
    $ taskfleet run list --output json | jq -r '(.data.runs // .data)[]
...
reading manifests by hand:
    ...
    $ jq -c . ~/.taskfleet/runs/01m05tmcnn7kfs0cye2kq1a7nb/manifest.json
...
Side observation, possibly related and possibly its own bug: `taskfleet node show n-0001
--run-id <id> --output json` produced no output at all for a completed run, while
`~/.taskfleet/runs/<id>/nodes/n-0001.json` existed and was readable.
