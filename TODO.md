...
- 🟡 **Skill-bundling campaign** [`skill-bundling-campaign`](issues/skill-bundling-campaign/item.md) — **9/10 phases authored**. Phases 1, 2, 3, 4a–d, 5, 6 merged: all `worktree-*`, `fan-out`, and `/orchestrate` bundled SKILL.md files committed, registry/tests/snapshots updated, build/clippy/fmt clean, `skill print`/`skill install --dest ...` smoke-pass. Contract template (`worktree-spinoff`, Phase 1) reviewed and approved by Jari before Phase 5. **Phase 7 (rollout: `skill install --all --force` over `~/.claude/skills/` + sunset homebase `~/.claude/skills/worktree-*`)** held: a parallel process is using the current homebase skills, so the catalog swap waits for that to finish.
